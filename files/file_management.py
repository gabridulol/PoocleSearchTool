...
def fileManagement(src_filename): # Declaração da função read_clean_file que recebe como parâmetro o diretório do arquivo de entrada -> "files/entrada.txt"
    ...
    with open(src_filename, 'r') as src_file: # Abertura e leitura do arquivo de entrada -> "entrada.txt"
        src_text = src_file.readlines() # Armazena o arquivo "entrada.txt" na lists src_text -> ['N\n', 'arquivo1.txt\n', 'arquivo2.txt\n', ... , 'arquivoN.txt']
        ...
        for i in range(1, len(src_text)): # Estrutura de repetição acessar os itens da lista src_text a partir do segundo elemento, acessando o nome dos arquivos
            ...
            with open(filename, 'r') as file: # Abre o arquivo específico desta iteração, "arquivoN.txt", no modo leitura
                ...
                text = [word for word in text if word.lower() not in stop_words] # Remove stopwords percorrendo a lista text e verificando se é uma stopword
